...
hermes dashboard            Launch web dashboard (requires `pip install 'hermes-agent[web]'`)
...
| `agent` | `max_turns` (90), `tool_use_enforcement`, `complexity_routing` (bool — enables auto-routing to coding model), `min_coding_score` (0.65) |
...
| `computer_use` | macOS desktop control via cua-driver (screenshots, click, drag, scroll, type, key, wait, list_apps, focus_app). **macOS only** — requires cua-driver on $PATH. Does NOT steal cursor/focus. |
...
> ⚠️ **Cron skill name pitfall:** The `skills` field in cron job creation
> expects **skill names** (e.g., `"browser"`), NOT toolset names (e.g., `"web"`).
> Using `skills: ["web"]` will silently fail with "Skill 'web' not found."
> Use `skills: ["browser"]` for web searching, or omit the `skills` field
> entirely to let the agent use its default toolset. To see available skills,
> run `hermes skills list` or check `skills_list()`.
...
### Updating Hermes on Windows

**Do NOT stop the gateway before updating.** Use the git checkout method instead. See `references/windows-update-procedure.md` for the full step-by-step.

Quick version:
```bash
cd C:\Users\<user>\AppData\Local\hermes\hermes-agent
git stash && git fetch origin && git checkout <latest-tag>
...
**Use the CLI, not the dashboard, for enabling/disabling plugins.** The dashboard's Enable buttons have no stable `data-ref` attributes and can't be reliably targeted via browser automation. The CLI is deterministic:
...
Plugin changes take effect on the next session (not mid-conversation).

**Free plugins worth enabling by default** (bundled, no API key needed):
- `disk-cleanup` — auto-cleans temp files, zero config
- `web/ddgs` — DuckDuckGo search, no API key
- `security-guidance` — warns about dangerous code patterns (eval, pickle, etc.)
- `web/brave_free` — Brave Search free tier (2k queries/month)
- `image_gen/fal` — FAL.ai image generation (free tier)
- `image_gen/xai` — xAI Grok image generation (free tier)

**Plugins that need paid API keys** (skip unless user has keys):
`browser/browser_use`, `browser/browserbase`, `browser/firecrawl`, `image_gen/openai`, `image_gen/openai-codex`, `image_gen/krea`, `web/exa`, `web/parallel`, `web/tavily`, `web/firecrawl`

See `references/windows-update-procedure.md` for the full update workflow.
...
### Vision / Image Input Not Working

If the user sends images but the agent can't see them (404 "No endpoints found that support image input"), the current model is text-only. See `references/model-vision-support.md` for model switching instructions and workarounds.
...
**Distinguishing old vs. current errors:** Log files accumulate across sessions. Always check timestamps on error entries. Errors from previous days are stale if the gateway was restarted today. Use date-filtered grep:
```bash
grep "2026-06-02" ~/.hermes/logs/errors.log | grep -i "error\|fail"
```

**⚠️ Do NOT restart the gateway autonomously.** The user controls gateway restarts. If the gateway appears stuck, diagnose and report findings — let the user decide when to restart. Restarting while a previous instance is still holding connections (especially Telegram polling) can cause connection conflicts.
...
### Pydantic-Core Version Mismatch (Windows)

**Symptom:** Telegram or agent responses show `pydantic-core X.Y.Z is incompatible, requires X.Y.W`, or the gateway fails to initialize the OpenAI client.

**Diagnosis:**
```bash
cd ~/.hermes/hermes-agent && python -c "import pydantic_core; print(pydantic_core.__version__)"
...
**Note:** Check log timestamps — this error may be from a previous session. If the current venv has matching versions, the error is stale.
...
The WhatsApp integration uses a **Node.js bridge** (Baileys) that runs as a separate process on `localhost:3000`. The Hermes gateway and the WhatsApp bridge are **independent** — the gateway can be running while the bridge is down.

#### Symptom: `send_message` fails with "Cannot connect to host localhost:3000"

The bridge process is not running. The PID file at `~/.hermes/whatsapp/session/bridge.pid` may be stale.

**Fix:**
```bash
# Kill stale PID (if any)
kill $(cat ~/.hermes/whatsapp/session/bridge.pid) 2>/dev/null

# Start the bridge (from the hermes-agent scripts directory)
cd ~/.hermes/hermes-agent/scripts/whatsapp-bridge
node bridge.js --port 3000 --session ~/.hermes/whatsapp/session --mode self-chat &

# Verify it's up
sleep 8 && curl -s http://localhost:3000/health
# Expected: {"status":"connected","queueLength":0,"uptime":...}
```

On Windows, the session path is `C:\Users\<user>\AppData\Local\hermes\whatsapp\session`.

#### Symptom: `send_message` fails with "Cannot destructure property 'user' of 'jidDecode(...)'"

The `send_message` tool's JID decoder fails on bare phone numbers. **Workaround:** send directly via the bridge's HTTP API:
...
This bypasses the gateway's JID decoding entirely. The bridge returns `{"success":true,"messageId":"..."}` on success.

#### Symptom: Bridge starts but immediately disconnects

- Check `~/.hermes/whatsapp/session/creds.json` exists and is valid
- If logged out, delete the session directory and re-pair with `hermes whatsapp`
- Check that `WHATSAPP_MODE` and `WHATSAPP_ALLOWED_USERS` are set in `.env`

#### Key paths (Windows)
- `.env`: `C:\Users\<user>\AppData\Local\hermes\.env`
- Session: `C:\Users\<user>\AppData\Local\hermes\whatsapp\session\`
...
Full bridge commands and troubleshooting: `references/whatsapp-bridge.md`
...
| Web Dashboard | `hermes dashboard` or [Dashboard docs](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard) |
| Memory | `hermes memory status` or [Memory docs](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory) |
| Env variables | `hermes config env-path` or [Env vars reference](https://hermes-agent.nousresearch.com/docs/reference/environment-variables) |
| Obsidian vault | `OBSIDIAN_VAULT_PATH` in config.yaml — `hermes config set env.OBSIDIAN_VAULT_PATH "<path>"` |
...
**Release notes:** v0.14.0 "The Foundation Release" summary — see [`references/release-notes-v0.14.0.md`](references/release-notes-v0.14.0.md).
