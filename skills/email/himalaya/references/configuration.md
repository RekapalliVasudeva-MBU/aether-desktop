...
- Windows: `%APPDATA%/himalaya/config.toml` (typically `C:\Users\<user>\AppData\Roaming\himalaya\config.toml`)
...
> **Gmail folder aliases are REQUIRED, not optional.** Gmail's IMAP folders use names like `[Gmail]/Sent Mail` instead of `Sent`. Without the alias block, `himalaya template send` will fail on save-to-Sent with "Folder doesn't exist" after SMTP delivery succeeds. The message is already sent, but himalaya exits non-zero. If any caller retries on that exit code, it re-runs SMTP — producing **duplicate emails**. Always include the `[Gmail]/Sent Mail` alias block for Gmail accounts.
