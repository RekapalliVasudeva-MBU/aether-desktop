...
- Pull and manage models via **Ollama** (simpler alternative to raw llama.cpp)

## ⚠️ Before Downloading Anything — Always Ask First

**Never run `ollama pull`, `ollama run`, or download models without explicit user confirmation.** Even if the user asks "will this model work on my config?" — answer the question first, then ask "want me to download it?" before starting any pull. Downloads consume disk space (5-10GB per model) and bandwidth. The user may have already downloaded it, may want a different quant, or may prefer to handle it themselves.

**CRITICAL: `ollama run` triggers a download if the model isn't local.** Never use it to "check specs" or "test" a model — use `ollama show <model>` for specs, or search the web for benchmarks.

**Correct flow:**
1. Research the model (benchmarks, size, capabilities) — use web search, HuggingFace API
2. Check GPU/VRAM compatibility (see "VRAM Budget Check" below)
3. Present findings + recommendation
4. **Ask:** "Want me to download this?"
5. Only then run `ollama pull <model>`
...
ollama run <model>           # Interactive chat — DOWNLOADS if not local!
ollama show <model>          # Show model details (safe, no download)
```

## VRAM Budget Check (Before Pulling)

Before recommending a local model, verify it fits the user's GPU. A model needs:
```
model_weights_size + context_window_size ≤ total_VRAM
```

**Quick check commands:**
```powershell
# Check current VRAM usage on Windows (NVIDIA)
nvidia-smi --query-gpu=name,memory.used,memory.total,memory.free --format=csv,noheader
```

**Rule of thumb for context window:**
- Minimum usable context: 2GB (very tight, short conversations)
- Comfortable: 4GB+ for context
- If model is 6.1GB on 8GB VRAM → only ~1.5GB left for context → works for vision tasks (images are tokenized, not stored in context) but tight for long text

**If it fits → proceed with pull. If borderline → tell the user and let them decide. If it doesn't fit → suggest cloud alternatives.**

## Diagnosing Slow/Stuck Ollama Downloads

`registry.ollama.ai` (Cloudflare R2) can be extremely slow on some networks (~400KB/s). A 4.4GB model at that speed takes 3+ hours.

**Diagnosis:**
```bash
# Check if download is progressing (watch the % over 30-60 seconds)
# If stuck at same % for 5+ minutes → throttled or connection dropped

# Test raw download speed to the registry
curl -sL -o /dev/null -w "speed: %{speed_download} bytes/s\n" --max-time 10 \
  "https://registry.ollama.ai/v2/library/minicpm-v/blobs/sha262843d4806a"
```

**If speed < 100KB/s:**
- Your ISP or region throttles Cloudflare R2 / Ollama registry
- Options: (1) let it run overnight, (2) use a VPN, (3) use cloud API instead

**Progress reporting:** When monitoring a download, report progress at intervals (e.g., every 5 minutes), not every few seconds. Don't give conflicting numbers — state the current % and estimated time remaining based on the last two samples.
...
## Choosing between local models (head-to-head comparison)

For a current side-by-side comparison of the best local models in the 8-9B class (Granite4.1, LFM2.5, qwythos), including sizing, capabilities, and recommendation matrix — see:

- **[local-model-comparison-2026.md](references/local-model-comparison-2026.md)**
...
- **[ollama-network-troubleshooting.md](references/ollama-network-troubleshooting.md)** — diagnosing slow/stuck Ollama downloads, registry throttling, VRAM budget checks, workarounds
